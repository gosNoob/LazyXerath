...
function LazyXerath:castingQ()
	...
	if self.chargeQ == false and myHero.activeSpell.name == "XerathArcanopulseChargeUp" and myHero.activeSpell.valid then
		self.qTick = GetTickCount()
		self.chargeQ = true
	end
	if self.chargeQ == true and myHero.activeSpell.name ~= "XerathArcanopulseChargeUp" and myHero.activeSpell.valid == false then
		self.chargeQ = false
		self.Q.range = 750
...
function LazyXerath:castingR()
	...
	if self.chargeR == false and myHero.activeSpell.name == "XerathLocusOfPower2" then
		self.chargeR = true
		self.chargeRTick = GetTickCount()
		self.firstRCast = true
	end
	if self.chargeR == true and myHero.activeSpell.name ~= "XerathLocusOfPower2" and myHero.activeSpell.valid == false then
		self.chargeR = false
		self.R_target = nil
	end
